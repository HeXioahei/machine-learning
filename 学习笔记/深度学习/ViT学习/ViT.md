...
再进行扁平化处理拉长为一个向量`[1,196,768]`，768是卷积中的通道数个数，也即即将进入embedding层的批量。196即为还没加上class token时的token数。
...
transformer encoder 层就是将encoder block 堆叠 L 层得到的。在encoder block中，先是通过了layer norm 层，