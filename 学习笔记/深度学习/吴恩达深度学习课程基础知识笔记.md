# logistic回归
## logistic回归
用于解决二分类问题，使得输出结果在0到1之间，满足概率的形式。就是将线性函数的结果再放入sigmoid函数中进行激活。
$$z=w^{T}x+b$$
$$\hat{y}=a=\sigma(z)$$
...
# 前向传播和反向传播
## 流程图（单个样本）
先进行前向传播，通过参数w和b计算出y_hat，与y比较，若差距较大，则进行逆向传播，利用梯度下降法更新参数，再继续前向传播，计算出y_hat，与y比较，反复进行，直到差距很小，即J很小为止。
...
**注意**：这里的x1和x2并不是代表两个样本，而是一个样本中的两个属性特征，也即一个样本中的两个变量。
