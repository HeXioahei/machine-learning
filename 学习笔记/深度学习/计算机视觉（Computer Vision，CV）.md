...
# 单层卷积网络
对于每一个卷积核，先进行卷积核的运算得到一张矩阵图，然后加上偏差bias（不同卷积核的bias不一样），然后进行relu函数激活，得到一张新的矩阵图。最终将这些矩阵图堆叠成三维立方体。这就是一层卷积网络了。
...
进行激活函数前的结果就是$z^{[1]}$，激活后的就是$a^{[1]}$。

用卷积的一大好处就是，即使图片特别大，卷积的参数也可以很少。